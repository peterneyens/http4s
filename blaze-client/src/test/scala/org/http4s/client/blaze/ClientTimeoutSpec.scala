...
class ClientTimeoutSpec extends Http4sSpec {
  ...
  def mkClient(head: => HeadStage[ByteBuffer], tail: => BlazeClientStage)
              (idleTimeout: Duration, requestTimeout: Duration): BlazeClient = {
  ...
  "Http1ClientStage responses" should {
    "Timeout immediately with an idle timeout of 0 seconds" in {
      ...
      c(FooRequest).void.run must throwA[TimeoutException]
    }
    ...
    "Request timeout on slow POST body" in {
      ...
      c(req).void.run must throwA[TimeoutException]
    }
    ...
    "Idle timeout on slow POST body" in {
      ...
      c(req).as[String].run must throwA[TimeoutException]
    }
    ...
    "Not timeout on only marginally slow POST body" in {
      ...
      c(req).as[String].run must_== ("done")
    }
    ...
    "Request timeout on slow response body" in {
      ...
      c(FooRequest).as[String].run must throwA[TimeoutException]
    }
